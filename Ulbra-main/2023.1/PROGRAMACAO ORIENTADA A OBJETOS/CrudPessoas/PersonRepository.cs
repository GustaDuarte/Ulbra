...
namespace CrudPessoas
{
    public class PersonRepository
    {
        ...
        public void ListPerson(){
            ...
            {
                Console.WriteLine($"{person.Id}\t{person.Name}\t{person.Phone}\t{person.City.Id}\t{person.City.Name}");
            }
        }

        public void UpdatePerson(int id, string name, string phone, string nameCity){
            Person person = people.Find(p => p.Id == id); //verifica se o atributo Id no objeto p é igual ao valor da variavel id;
            if (person != null)
            {
                person.Name = name;
                person.Phone = phone;
                person.City.Name = nameCity;
